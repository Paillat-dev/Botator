...
async def davinci_process(self, messages, message, api_key, max_tokens, temperature, frequency_penalty, presence_penalty, prompt):
    for msg in messages:
        content = msg.content
...
async def chat_process(self, messages, message, api_key, prompt, images_enabled, premium):
...
async def chat_process(self, message):
    ...
    except: model = "gpt-3.5-turbo" # if the guild is not in the database, we set the model to gpt-3.5-turbo
    if model.lower() == "chatgpt": model = "gpt-3.5-turbo" # if the model is chatGPT, we set the model to gpt-3.5-turbo
    ...
    if model == "chatGPT" or model == "gpt-4": # if the model is chatGPT, we handle it in a certain way
        #async def chat_process(self, messages, message, api_key, prompt, images_enabled, premium):
        response = await chat_process(self, messages, message, api_key, prompt, images_enabled, premium)
#-----------------------------------------Davinci------------------------------------------------------------------------------------------


    elif model == "davinci":
        response = await davinci_process(self, messages, message, api_key, max_tokens, temperature, frequency_penalty, presence_penalty, prompt)
    if response != "":
        if tts: tts = True
        else: tts = False
